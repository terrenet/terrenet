...
var World = {
   ...
   tipo: new Array(),
	...
	loadPoisFromJsonData: function loadPoisFromJsonDataFn(poiData) {
		...
		World.updateStatusMessage(currentPlaceNr + ' POIs caricati');
	...
	showRange: function showRangeFn() {
		if (World.markerList.length > 0) {
			...
			World.updateStatusMessage('Nessun POI trovato', true);
		}
	},
	...
	requestDataFromServer: function requestDataFromServerFn(lat, lon, tipo) {
      //alert(lat+' - '+lon);
		...
		World.updateStatusMessage('Caricamento POIs');
      //alert(lat+" - "+lon);
      //lat = 41.064759;
      //lon = 15.251768;
      //alert(lat+" - "+lon);
      ...
      if(tipo.length > 0){
		...
		var jqxhr = $.getJSON(serverUrl, function(data) {
			...
			.error(function(err) {
				World.updateStatusMessage("Risposta non valida.", true);
